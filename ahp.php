...
function ahp_get_matrik_kriteria_ukm($id_kriteria_ukm){
...
function ahp_get_matrik_ukm($id_kriteria_ukm, $id_ukm){
	for($i=0;$i<count($id_ukm);$i++){
		for($ii=0;$ii<count($id_ukm);$ii++){
			if($i==$ii){
				...
				if($i < $ii){
					$q=mysql_query("select nilai from nilai_ukm where id_kriteria_ukm='".$id_kriteria_ukm."' and id_ukm_1='".$id_ukm[$i]."' and id_ukm_2='".$id_ukm[$ii]."'");
					if(mysql_num_rows($q)>0){
						$h=mysql_fetch_array($q);
						$nilai=$h['nilai'];
						$matrik[$i][$ii]=$nilai;
